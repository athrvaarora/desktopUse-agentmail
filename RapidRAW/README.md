...
# RapidRAW - AI-Navigable Desktop Edition

> A beautiful, non-destructive, and GPU-accelerated RAW image editor with complete LLM navigation framework.

RapidRAW is a modern, high-performance alternative to Adobe Lightroom®. This fork demonstrates a complete implementation of **Desktop Use** - enabling AI agents to control the entire application through natural language commands.

**Original Project**: This is a fork of [CyberTimon/RapidRAW](https://github.com/CyberTimon/RapidRAW), an incredible project developed by an 18-year-old developer as a personal challenge. This implementation adds comprehensive AI navigation capabilities while preserving all original functionality.

## 🤖 Desktop Use Implementation

This version of RapidRAW features a complete **LLM navigation framework** that makes every UI element controllable by AI agents. Through the [Desktop Use SDK](../desktop-control-sdk), agents can:

- Navigate all adjustment panels (Basic, Curves, Color, Details, Effects)
- Create and manage complex mask hierarchies
- Control brush tools and mask components
- Execute batch operations and exports
- Browse the image library
- Apply presets and copy/paste settings

### Navigation Coverage

- **70+ Tagged Components**: Every button, slider, switch, and input
- **Dynamic Elements**: Mask containers, SubMasks, and adjustment sections
- **Context-Aware Actions**: Custom actions per component type
- **Multi-Step Workflows**: Complex editing sequences via natural language

### Technical Implementation

The implementation uses the **Desktop Use SDK** to tag components:

```typescript
// Example: Brightness slider with AI navigation
  ...
  customActions: {
    setValue: (newValue: any) => {
      ...
      if (typeof numValue === 'number' && !isNaN(numValue)) {
        ...
        handleAdjustmentChange('brightness', clampedValue);
      }
    },
    increase: (amount: any) => { /* ... */ },
    decrease: (amount: any) => { /* ... */ }
...
// Wrap component with ref
return <div ref={brightnessRef}><Slider ... /></div>;
```

**Key Pattern**: All navigation refs are attached to **wrapper divs**, not directly to components. This ensures consistent behavior across all UI elements.

### Complete UI Sitemap

A comprehensive sitemap is maintained using **Hyperspell** memory and stored in Notion:

📄 **[View Complete RapidRAW UI Sitemap](https://cerulean-visage-643.notion.site/RapidRAW-UI-Sitemap-Complete-LLM-Navigation-Framework-dd0b8dd1d07941909a22a4f9bcfb287d)**
...
- Remember the complete UI structure across sessions
- Navigate efficiently without rediscovery
- Execute complex multi-step operations
- Understand component relationships and context
...
To run this Desktop Use enabled version, you'll need:
- [Rust](https://www.rust-lang.org/tools/install) toolchain
- [Node.js](https://nodejs.org/) (v18 or newer)
- MCP Server running (see [../mcp-server](../mcp-server))

### Building from Source

```bash
# 1. Clone the repository
git clone https://github.com/athrvaarora/desktopUse-agentmail.git
cd desktopUse-agentmail/RapidRAW
...
1. **Start the MCP Server**: See [MCP Server deployment guide](../mcp-server/REPLIT_DEPLOYMENT.md)
2. **Launch RapidRAW**: The app automatically connects to the MCP server via WebSocket
3. **Use Claude Desktop**: Connect Claude to the MCP server and start issuing commands
...
"Export the current image as JPEG"
```

### Original RapidRAW

For the standalone version without AI navigation:
- Visit the [original repository](https://github.com/CyberTimon/RapidRAW)
- Download pre-built releases for Windows, macOS, and Linux
- Available on Flathub and AUR for Linux users
...
## Desktop Use Architecture

This implementation demonstrates several key patterns for making desktop applications AI-navigable:

### 1. Component Tagging Pattern
Every interactive element is wrapped with navigation tags:
```typescript
const { ref } = useNavigation({ id, type, label, availableActions, metadata, customActions });
return <div ref={ref}><Component /></div>;
```
## Related Projects

- **[Desktop Use Agent Mail](../README.md)**: Parent project with MCP server and SDK
- **[Original RapidRAW](https://github.com/CyberTimon/RapidRAW)**: The base application
- **[Desktop Use SDK](../desktop-control-sdk)**: React navigation hooks

## Support the Original Project

If you find RapidRAW useful, please support the original creator:

**CyberTimon** (18-year-old developer):
-   **Ko-fi:** [Donate on Ko-fi](https://ko-fi.com/cybertimon)
-   **Crypto:**
    -   BTC: `36yHjo2dkBwQ63p3YwtqoYAohoZhhUTkCJ`
    -   ETH: `0x597e6bdb97f3d0f1602b5efc8f3b7beb21eaf74a`
    -   SOL: `CkXM3C777S8iJX9h3MGSfwGxb85Yx7GHmynQUFSbZXUL`

## License

This project maintains the **GNU Affero General Public License v3.0 (AGPL-3.0)** from the original RapidRAW project. The Desktop Use additions are also AGPL-3.0 licensed to ensure the entire framework remains open-source and free for the community.

See the [LICENSE](../LICENSE) file for more details.

## Author

**Desktop Use Implementation**: Athrva Arora (@athrvaarora)

**Original RapidRAW**: CyberTimon ([GitHub](https://github.com/CyberTimon))
const MaskContainer = ({ mask }) => {
  const { ref } = useNavigation({ id: `mask-${mask.id}`, ... });
  return <button ref={ref}>...</button>;
};
```

### 4. Consistent Patterns Across Panels
All adjustment panels (Basic, Color, Curves, Details, Effects, Masks) follow identical tagging patterns for consistency.

## Technology Stack

**Original RapidRAW:**
- **Backend**: Rust (Tauri, WGPU, rawler)
- **Frontend**: React, TypeScript
- **UI**: Custom WGSL shaders for GPU processing

**Desktop Use Additions:**
- **Navigation SDK**: Desktop Use SDK (React hooks)
- **MCP Server**: TypeScript, WebSocket
- **Memory**: Hyperspell + Notion integration
...
*   **[CyberTimon/RapidRAW](https://github.com/CyberTimon/RapidRAW):** For the incredible base application that made this implementation possible. An 18-year-old developer built a professional-grade photo editor from scratch - truly impressive work.
*   **[Google AI Studio](https://aistudio.google.com):** For providing amazing assistance in researching and implementing image processing algorithms.
*   **[rawler](https://github.com/dnglab/dnglab/tree/main/rawler):** For the excellent Rust crate that provides the foundation for RAW file processing.
*   **[Anthropic](https://www.anthropic.com):** For Claude and the Model Context Protocol.
*   **[Replit](https://replit.com):** For providing reliable hosting for the MCP server.
*   **[Hyperspell](https://hyperspell.com):** For the memory management framework.
