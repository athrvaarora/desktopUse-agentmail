...
export default function Controls({
  ...
  // Tag Auto Adjust button
  const { ref: autoAdjustRef } = useNavigation({
    id: 'auto-adjust-button',
    type: 'button',
    label: 'Auto Adjust Button',
    availableActions: ['click'],
    metadata: {
      description: 'Automatically calculate and apply optimal adjustments to the image',
      disabled: !selectedImage,
      hasImage: !!selectedImage,
    },
    customActions: {
      apply: async () => {
        console.log('[AutoAdjust] apply() called, selectedImage:', !!selectedImage);
        if (selectedImage) {
          try {
            await handleAutoAdjustments();
            console.log('[AutoAdjust] apply() completed successfully');
          } catch (error) {
            console.error('[AutoAdjust] apply() failed:', error);
            throw error;
          }
        } else {
          console.warn('[AutoAdjust] No image selected');
        }
      },
      autoAdjust: async () => {
        console.log('[AutoAdjust] autoAdjust() called, selectedImage:', !!selectedImage);
        if (selectedImage) {
          try {
            await handleAutoAdjustments();
            console.log('[AutoAdjust] autoAdjust() completed successfully');
          } catch (error) {
            console.error('[AutoAdjust] autoAdjust() failed:', error);
            throw error;
          }
        } else {
          console.warn('[AutoAdjust] No image selected');
  ...
  const { ref: resetAllRef } = useNavigation({
    id: 'reset-all-adjustments-button',
    type: 'button',
    label: 'Reset All Adjustments Button',
    availableActions: ['click'],
    metadata: {
      description: 'Reset all adjustments to their default values',
      disabled: !selectedImage,
    },
    customActions: {
      reset: () => {
