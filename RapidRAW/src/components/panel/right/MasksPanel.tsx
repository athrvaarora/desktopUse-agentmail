...
// Component for mask container control buttons with navigation tags
const MaskContainerControls = ({
  container,
  onToggleVisibility,
  onDelete,
}: {
  ...
  const { ref: visibilityButtonRef } = useNavigation({
    id: `mask-container-visibility-${container.id}`,
    type: 'button',
    label: `${container.visible ? 'Hide' : 'Show'} Mask Container`,
    availableActions: ['click'],
    metadata: {
      description: `Toggle visibility for mask container "${container.name}", currently ${container.visible ? 'visible' : 'hidden'}`,
  ...
  const { ref: deleteButtonRef } = useNavigation({
    id: `mask-container-delete-${container.id}`,
    type: 'button',
    label: `Delete Mask Container`,
    availableActions: ['click'],
    metadata: {
      description: `Delete mask container "${container.name}"`,
      containerId: container.id,
      containerName: container.name,
    },
    customActions: {
...
export default function MasksPanel({
  ...
  // Navigation tags for header buttons
  const { ref: backButtonRef } = useNavigation({
    id: 'mask-back-to-list-button',
    type: 'button',
    label: 'Back to Mask List',
    availableActions: ['click'],
    metadata: {
      description: 'Return to mask list view from editing view',
    },
    customActions: {
  ...
  const { ref: resetMaskAdjustmentsButtonRef } = useNavigation({
    id: 'mask-reset-adjustments-button',
    type: 'button',
    label: 'Reset Mask Adjustments',
    availableActions: ['click'],
    metadata: {
      description: 'Reset all adjustments for the current mask to default values',
    },
    customActions: {
      click: () => {
        const container = adjustments.masks.find((m: MaskContainer) => m.id === activeMaskContainerId);
        if (container) {
          updateContainer(container.id, { adjustments: { ...INITIAL_MASK_ADJUSTMENTS } });
        }
      },
    },
  });

  // Navigation tags for Create Mask buttons
  const createSubjectMaskNav = useNavigation({
    id: 'create-subject-mask-button',
    type: 'button',
    label: 'Create Subject Mask',
    availableActions: ['click'],
    metadata: {
      description: 'Create a new mask container with AI-detected subject mask component',
    },
    customActions: {
      click: () => handleAddMaskContainer(Mask.AiSubject),
    },
  });

  const createSkyMaskNav = useNavigation({
    id: 'create-sky-mask-button',
    type: 'button',
    label: 'Create Sky Mask',
    availableActions: ['click'],
    metadata: {
      description: 'Create a new mask container with AI-detected sky mask component',
    },
    customActions: {
      click: () => handleAddMaskContainer(Mask.AiSky),
    },
  });

  const createForegroundMaskNav = useNavigation({
    id: 'create-foreground-mask-button',
    type: 'button',
    label: 'Create Foreground Mask',
    availableActions: ['click'],
    metadata: {
      description: 'Create a new mask container with AI-detected foreground mask component',
    },
    customActions: {
      click: () => handleAddMaskContainer(Mask.AiForeground),
    },
  });

  const createLinearMaskNav = useNavigation({
    id: 'create-linear-mask-button',
    type: 'button',
    label: 'Create Linear Mask',
    availableActions: ['click'],
    metadata: {
      description: 'Create a new mask container with linear gradient mask component',
    },
    customActions: {
      click: () => handleAddMaskContainer(Mask.Linear),
    },
  });

  const createRadialMaskNav = useNavigation({
    id: 'create-radial-mask-button',
    type: 'button',
    label: 'Create Radial Mask',
    availableActions: ['click'],
    metadata: {
      description: 'Create a new mask container with radial gradient mask component',
    },
    customActions: {
      click: () => handleAddMaskContainer(Mask.Radial),
    },
  });

  const createBrushMaskNav = useNavigation({
    id: 'create-brush-mask-button',
    type: 'button',
    label: 'Create Brush Mask',
    availableActions: ['click'],
    metadata: {
      description: 'Create a new mask container with brush-painted mask component',
    },
    customActions: {
      click: () => handleAddMaskContainer(Mask.Brush),
    },
  });

  const getCreateMaskButtonRef = (maskType: MaskType) => {
    switch (maskType.type) {
      case Mask.AiSubject:
        return createSubjectMaskNav.ref;
      case Mask.AiSky:
        return createSkyMaskNav.ref;
      case Mask.AiForeground:
        return createForegroundMaskNav.ref;
      case Mask.Linear:
        return createLinearMaskNav.ref;
      case Mask.Radial:
        return createRadialMaskNav.ref;
      case Mask.Brush:
        return createBrushMaskNav.ref;
      default:
        return undefined;
    }
  };
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="p-4 flex justify-between items-center flex-shrink-0 border-b border-surface h-[69px]">
          ...
          <button
            ref={resetMaskAdjustmentsButtonRef as any}
            className="p-2 rounded-full hover:bg-surface transition-colors flex-shrink-0"
        ...
        <div onClick={(e: any) => e.stopPropagation()}>
          ...
          <div className="grid grid-cols-3 gap-2">
            {MASK_PANEL_CREATION_TYPES.map((maskType: MaskType) => (
              <button
                ref={getCreateMaskButtonRef(maskType) as any}
                ...
                {adjustments.masks
                  ...
                  .map((container: MaskContainer, index: number) => (
                    ...
                    >
                      ...
                      <MaskContainerControls
                        container={container}
                        onToggleVisibility={() => handleToggleContainerVisibility(container.id)}
