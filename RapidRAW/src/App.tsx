...
function App() {
  ...
  useEffect(() => {
    ...
    const generate = async () => {
      if (showOriginal && selectedImage?.path && !transformedOriginalUrl) {
        try {
          const imageData = await invoke('generate_original_transformed_preview', {
    ...
    invoke(Invokes.LoadSettings)
      .then(async (settings: any) => {
        ...
        if (settings?.pinnedFolders && settings.pinnedFolders.length > 0) {
          try {
            const trees = await invoke(Invokes.GetPinnedFolderTrees, { paths: settings.pinnedFolders }) as any[];
  ...
  const handleTogglePinFolder = useCallback(async (path: string) => {
    ...
    try {
      const trees = await invoke(Invokes.GetPinnedFolderTrees, { paths: newPins }) as any[];
      setPinnedFolderTrees(trees);
    } catch (err) {
  ...
  const handleSelectSubfolder = useCallback(
    async (path: string | null, isNewRoot = false) => {
      ...
      try {
        ...
        if (isNewRoot) {
          setExpandedFolders(new Set([path].filter((p): p is string => p !== null)));
        } else if (path) {
          setExpandedFolders((prev) => {
            const newSet = new Set(prev);
            const allRoots = [rootPath, ...pinnedFolders].filter((p): p is string => Boolean(p));
  ...
  useEffect(() => {
    ...
    const generate = async () => {
      ...
      try {
        const imageData = await invoke(Invokes.GenerateFullscreenPreview, { jsAdjustments: adjustments }) as Uint8Array;
        const blob = new Blob([imageData as unknown as BlobPart], { type: 'image/jpeg' });
  ...
  const handlePasteAdjustments = useCallback(
    (paths?: Array<string>) => {
      ...
      const { mode, includedAdjustments } = (appSettings as any).copyPasteSettings || { mode: 'merge', includedAdjustments: [] };
      ...
      invoke(Invokes.GenerateFullscreenPreview, {
        ...
        .then((imageData) => {
          if (!request.cancelled) {
            const blob = new Blob([(imageData as Uint8Array) as unknown as BlobPart], { type: 'image/jpeg' });
  ...
  useEffect(() => {
    ...
    const loadFullImageData = async () => {
        ...
        if ((appSettings as any)?.editorPreviewResolution) {
            const maxSize = (appSettings as any).editorPreviewResolution;
  ...
  }, [selectedImage?.path, selectedImage?.isReady, resetAdjustmentsHistory, (appSettings as any)?.editorPreviewResolution]);
  ...
  return (
    <div
      className={clsx(
        'flex flex-row h-screen bg-bg-primary font-sans text-text-primary overflow-hidden select-none',
    ...
    >
      ...
      <CopyPasteSettingsModal
        ...
        settings={(appSettings as any)?.copyPasteSettings as CopyPasteSettings}
        onSave={(newSettings) => handleSettingsChange({ ...appSettings, copyPasteSettings: newSettings } as any)}
      />
      <PanoramaModal
      ...
      {!isChatOpen && <ChatButton onClick={() => setIsChatOpen(true)} isOpen={isChatOpen} />}
      </div>
...
const AppWrapper = () => {
  // MCP Server WebSocket URL from environment (Replit deployment)
  const mcpServerUrl = import.meta.env.VITE_MCP_SERVER_URL || 'wss://desktopuse.replit.app';

  console.log('[AppWrapper] Rendering with MCP URL:', mcpServerUrl);
  console.log('[AppWrapper] Environment variables:', {
    VITE_MCP_SERVER_URL: import.meta.env.VITE_MCP_SERVER_URL,
    ...
    <ClerkProvider publishableKey={CLERK_PUBLISHABLE_KEY}>
      <ContextMenuProvider>
        <NavigationProvider 
          ...
          syncInterval={100}
          onConnect={() => {
            console.log('✅ [MCP] Connected to', mcpServerUrl);
            console.log('✅ [MCP] Connection successful - WebSocket is open');
          }}
          onDisconnect={() => {
            console.warn('⚠️ [MCP] Disconnected from', mcpServerUrl);
          }}
          onError={(err) => {
            console.error('❌ [MCP] Error connecting to', mcpServerUrl);
            console.error('❌ [MCP] Error details:', err);
            console.error('❌ [MCP] Make sure the MCP server is running at:', mcpServerUrl);
          }}
          onGraphUpdate={(graph) => {
            console.log('📊 [MCP] Component graph updated:', graph.nodes.size, 'components');
          }}
        >
          <App />
        </NavigationProvider>
